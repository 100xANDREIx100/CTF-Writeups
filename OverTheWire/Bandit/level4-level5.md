...
## 🎯 Objective
Find the password in the human readdable file
...
On each on them I run the same command in order to find the one with human redable data.

```bash
file "./-file00"
```
There I saw that all were `data` files except for one which was `ASCII text`. I readed that one (with `cat`).
